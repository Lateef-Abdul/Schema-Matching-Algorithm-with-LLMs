...
def compact_payload(full_payload):
    ...
    for src in full_payload["sources"]:
        ...
        tgt_tables = {cand["table"] for cand in src["candidates"]}
        compact["schema_context"]["sources_summary"].setdefault(src_table, set()).add(src["source"]["field"])
        for t in tgt_tables:
            compact["schema_context"]["targets_summary"].setdefault(t, set())
    ...
    for src in full_payload["sources"]:
        src_info = src["source"]
        # Trim neighbors and samples
        neighbors = [normalize_name(n.split()[-1]) for n in src_info.get("neighbors", [])[:5]]
        sample = next((s for s in src_info.get("samples", []) if s), None)
        src_entry = {
        ...
        for c in src["candidates"]:
            sim = c.get("signals", {})
            similarity = sum(sim.values()) / len(sim) if sim else None
            cand_sample = next((s for s in c.get("samples", []) if s), None)
            candidates.append({
                "table": c["table"],
                "field": c["field"],
                "type": c.get("type", ""),
